package com.blaze.runner.Modules.Interface;
...
import static com.blaze.runner.Libary.Converters.*;

public final class Interface implements Module {
    ...
    private enum Events {
        ...
        KEY_PRESSED(javafx.scene.input.KeyEvent.KEY_PRESSED),
        KEY_RELEASED(javafx.scene.input.KeyEvent.KEY_RELEASED),
        KEY_TYPED(javafx.scene.input.KeyEvent.KEY_TYPED);


        private final EventType<? extends javafx.event.Event> handler;

        private Events(EventType<? extends javafx.event.Event> handler) {
            this.handler = handler;
        }

        public EventType<? extends javafx.event.Event> getHandler() {
    ...
    public static void initConstants() {
        // Color class
        final Map<Value, Value> colors = Arrays.stream(javafx.scene.paint.Color.class.getDeclaredFields())
                .filter(f -> Modifier.isStatic(f.getModifiers()))
                .filter(f -> f.getType().equals(javafx.scene.paint.Color.class))
                .collect(Collectors.toMap(
                        f -> new StringValue(f.getName()),
                        f -> {
                            try {
                                return new ColorValue((javafx.scene.paint.Color) f.get(javafx.scene.paint.Color.class));
    ...
    public void init() {
        initConstants();
        Functions.set("Frame", new CreateWindow());

        MapValue EffectsMap = new MapValue(5);
        EffectsMap.set("Blend", new BlendEffect());
        EffectsMap.set("Bloom", new BloomEffect());
        EffectsMap.set("BoxBlur", new BoxBlurEffect());
        EffectsMap.set("ColorAdjust", new ColorAdjustEffect());
        EffectsMap.set("ColorInput", new ColorInputEffect());
        EffectsMap.set("DropShadow", new DropShadowEffect());
        EffectsMap.set("GaussianBlur", new GaussianBlurEffect());
        EffectsMap.set("Glow", new GlowEffect());
        EffectsMap.set("InnerShadow", new InnerShadowEffect());
        EffectsMap.set("Lighting", new LightingEffect());
        EffectsMap.set("MotionBlur", new MotionBlurEffect());
        EffectsMap.set("PerspectiveTransform", new PerspectiveTransformEffect());
        EffectsMap.set("Reflection", new ReflectionEffect());
        EffectsMap.set("SepiaTone", new SepiaToneEffect());
        EffectsMap.set("Shadow", new ShadowEffect());
        Variables.define("Effects", EffectsMap);

        Functions.set("Filter", new addEventFilter());
        Functions.set("Handler", new addEventHandler());
        Functions.set("Image", new createImage());
    }
    ...
    private static class ColorValue implements Value {
        ...
        public ColorValue(javafx.scene.paint.Color effect) {
            this.color = effect;
        }
    ...
    private static class rgbColor implements Function {
        ...
        public Value execute(Value... args) {
            ...
            return new ColorValue(javafx.scene.paint.Color.rgb(r, g, b, opacity));
        }
    }
    ...
    private static class hsbColor implements Function {
        ...
        public Value execute(Value... args) {
            ...
            double opacity = (args.length >= 4) ? args[3].asNumber() : 1d;
            return new ColorValue(javafx.scene.paint.Color.hsb(h, s, b, opacity));
        }
    }
    ...
    private static class webColor implements Function {
        ...
        @Override
        public Value execute(Value... args) {
            return new ColorValue(javafx.scene.paint.Color.web(args[0].asString(),
    ...
    private static class ColorInputEffect implements Function {
        ...
        public Value execute(Value... args) {
            return new EffectValue(new ColorInput(
                    args[0].asNumber(), args[1].asNumber(), args[2].asNumber(), args[3].asNumber(), (javafx.scene.paint.Color) args[4].raw()));
        }
    }
    ...
    private static class DropShadowEffect implements Function {
        ...
        public Value execute(Value... args) {
            ...
            switch (args.length) {
                case 2:
                    effect = new DropShadow(args[0].asNumber(), (javafx.scene.paint.Color) args[1].raw());
                    break;
                case 4:
                    effect = new DropShadow(args[0].asNumber(),
                            args[1].asInt(), args[2].asInt(),
                            (javafx.scene.paint.Color) args[3].raw());
                    break;
                case 6:
                    effect = new DropShadow(BlurType.values()[args[0].asInt()], (javafx.scene.paint.Color) args[1].raw(),
    ...
    private static class InnerShadowEffect implements Function {
        ...
        public Value execute(Value... args) {
            ...
            switch (args.length) {
                case 2:
                    effect = new InnerShadow(args[0].asNumber(), (javafx.scene.paint.Color) args[1].raw());
                    break;
                case 4:
                    ...
                    break;
                case 6:
                    effect = new InnerShadow(BlurType.values()[args[0].asInt()], (javafx.scene.paint.Color) args[1].raw(),
                            args[2].asNumber(), args[3].asNumber(), args[4].asNumber(), args[5].asNumber());
                    break;
                default:
    ...
    private static class LightingEffect implements Function {
        ...
        public Value execute(Value... args) {
            ...
            switch (l.size()) {
                case 3:
                    light = new Light.Distant(l.get(0).asNumber(), l.get(1).asNumber(), (javafx.scene.paint.Color) l.get(2).raw());
                    break;
                case 4:
                    light = new Light.Point(l.get(0).asNumber(), l.get(1).asNumber(), l.get(2).asNumber(), (javafx.scene.paint.Color) l.get(3).raw());
                    break;
                case 5:
                    light = new Light.Spot(l.get(0).asNumber(), l.get(1).asNumber(), l.get(2).asNumber(), l.get(3).asNumber(), (javafx.scene.paint.Color) l.get(4).raw());
                    break;
                default:
                    light = null;
            }
    ...
    private static class ShadowEffect implements Function {
        ...
        public Value execute(Value... args) {
            ...
            switch (args.length) {
                case 2:
                    effect = new Shadow(args[0].asNumber(), (javafx.scene.paint.Color) args[1].raw());
                    break;
                case 3:
                    effect = new Shadow(BlurType.values()[args[0].asInt()], (javafx.scene.paint.Color) args[1].raw(),
                            args[2].asNumber());
                    break;
                default:
                    effect = new Shadow();
...
//</editor-fold>
    ...
    public static class GraphicsFXValue extends MapValue {
        ...
        private void init() {
            ...
            set("paint", new Redraw());
            set("setTextAlign", this::setTextAlign);
            set("setTextBaseline", this::setTextBaseline);
            set("stroke", voidToVoid(graphics::stroke));
            set("create_line", double4ToVoid(graphics::strokeLine));
            set("create_oval", double4ToVoid(graphics::strokeOval));
            set("create_polygon", this::LPolygon);
            set("create_rect", double4ToVoid(graphics::strokeRect));
            ...
            set("FillPolygon", this::LFPolygon);
            set("FillRect", double4ToVoid(graphics::fillRect));
            set("create_text", this::LText);
        ...
        private Value LFill(Value... args) {
            graphics.setFill((javafx.scene.paint.Color) args[0].raw());
            return NumberValue.ZERO;
        }
        ...
        private Value setStroke(Value... args) {
            graphics.setStroke((javafx.scene.paint.Color) args[0].raw());
            return NumberValue.ZERO;
        }
    ...
    private static class CreateWindow implements Function {
        ...
        public Value execute(Value... args) {
            ...
            ImageIcon icon = new ImageIcon("Icons\\interface.png");
    ...
    private static void handleEvent(Event event, final Function handler) {
        if (event instanceof javafx.scene.input.MouseEvent) {
            handleMouseEvent((javafx.scene.input.MouseEvent) event, handler);
        } else if (event instanceof javafx.scene.input.KeyEvent) {
            handleKeyEvent((javafx.scene.input.KeyEvent) event, handler);
        }
    }
